...
| Access | Nick has guest access (nd@nickdawson.net). Fable will need its own path in — see §5. |
...
The MySQL server has **no public access** (private endpoint only). Practical implications for Fable:
- You **cannot** connect to the DB from a local machine or a plain CI runner. Options: (a) run schema setup + the `nrm-import.php` import **from inside the App Service** (Kudu/SSH console or a startup script once the container is VNet-integrated); (b) temporarily enable a public access firewall rule on the flexible server for migration, then disable it; (c) use a jump path on the VNet.
...
## 5. What Fable needs from Nick / Sean (not in this doc — secrets)

- [ ] **MySQL admin password** for `nqnssouudx` (from Sean, or Nick resets it via portal "Reset password").
- [ ] **Deployment access** to the App Service: either publish profile / deployment credentials, or add Fable's identity as a Contributor on the resource, or Nick runs the deploy steps.
- [ ] Decision on **registry** (spin up an Azure Container Registry in the RG, or use Docker Hub).
- [ ] Confirm whether Nick wants **VNet-integration verified / public DB rule** approach for the one-time import.

---

## 6. ⚠️ Cost flag to resolve with Sean (independent of the deploy)

- The **MySQL is General Purpose D2ds_v4** (~$130–190/mo) — the single thing breaking the ~$40/mo target. Ask Sean to **downsize to a Burstable tier (B1ms or B2s, ~$15–35/mo)**; plenty for this workload. Best to do **before** data is loaded.
